...
const ResumeVersion = require("../model/resumeVersion")
...
router.get(
    ...
    asyncHandler(async (req, res) => {
        ...
        if (latestResumeMeta) {
            ...
            versionStack = last3.map((v, i, arr) => {
                ...
                return {
                    id: v.id,
                    label: v.label,
                    title: v.sourceType === "upload" ? "Upload" : v.sourceType === "rewrite" ? "Rewrite pass" : v.label,
                    score: v.score ?? 0,
                    delta
                }
            })
        ...
        const versionsSpark = resumes.slice(0, 10).reverse().map((r) => ({ v: r.latestVersionNumber || 1 }))
        const keywordsSpark = allAnalyses.slice(-10).map((a) => ({ v: (a.keywordsPresent || []).length }))
        const issuesSpark = allAnalyses.slice(-10).map((a) => ({ v: (a.issues || []).length }))
